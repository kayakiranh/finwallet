...
{
    /// <summary>TR: Internal banka hesabını kimlik ve owner customer ile birlikte yükler. EN: Loads an internal bank account by identifier together with its owner-customer identifier.</summary>
    ...
    /// <summary>TR: Customer'ın belirtilen wallet'ına bağlı mevcut BankAccount kaydını arar. EN: Finds an existing BankAccount linked to the specified wallet of a customer.</summary>
    ...
    /// <summary>TR: Yeni Opening BankAccount kaydını eklemeyi dener; aynı Wallet için concurrent create yarışını false sonucu ile bildirir. EN: Attempts to insert a new Opening BankAccount and reports a concurrent same-Wallet create race as false.</summary>
    ...
    /// <returns>TR: Insert başarılıysa true; başka kayıt yarışı kazandıysa false döndürür. EN: Returns true when inserted; false when another record won the race.</returns>
    Task<bool> TryInsertAsync(BankAccount bankAccount, CancellationToken cancellationToken);

    /// <summary>
    /// TR: BankAccount state'ini beklenen status ve UpdatedAt snapshot koşullarıyla compare-and-set biçiminde günceller; aynı status içindeki stale provider-state write'larını da engeller.
    /// EN: Updates BankAccount state using compare-and-set semantics against expected status and UpdatedAt snapshot, also preventing stale provider-state writes within the same status.
    /// </summary>
    ...
    /// <param name="expectedUpdatedAt">TR: UPDATE öncesinde DB'de beklenen UpdatedAt snapshot değeri. EN: UpdatedAt snapshot expected in the database before UPDATE.</param>
    ...
    Task<bool> TryUpdateAsync(BankAccount bankAccount, BankAccountStatus expectedStatus, DateTimeOffset expectedUpdatedAt, CancellationToken cancellationToken);
}
